...
- 本项目代码会托管到 GitHub。
- 电脑 A 会从 GitHub clone 本项目并执行，因此项目必须包含清晰的依赖文件、配置示例和启动说明。
- 微信公众号原始数据由电脑 A 上已经部署的 Docker 服务定时采集、缓存并提供查询接口。本项目也部署在电脑 A 上，不负责部署该 Docker 服务，只通过其本机 REST API 查询数据。
