...
筛选标准：
- 最高优先级关注“客户端 + AI”方向，包括操作系统级 AI 能力、端侧 AI、系统级智能体、超级 App + AI、第三方厂商/开发者接入指南、插件生态、App 接入能力、新客户端 AI 产品或新功能。
- “客户端 + AI”示例：苹果发布系统级 AI 能力；小米发布系统级 AI 产品或 HyperOS AI 能力；千问/通义千问发布第三方厂商接入指南；豆包、微信、支付宝、钉钉、飞书等超级 App 发布 AI 能力或开放接入。
- 优先选择：大模型、智能体、AI 应用落地、开源模型/工具、开发者工具、AI 基础设施、重要产品发布、智能硬件、机器人、芯片、终端设备。
- 特别关注：Agent/智能体进展，新的开源技术或模型，重要公司产品能力更新，能影响开发者或行业应用的事件。
- 降低优先级或剔除：纯融资、IPO、股价、市值、招聘、裁员、会议报名、征稿、纯营销软文、无事实增量观点文、政治宏观内容、娱乐八卦。
- 如果融资新闻同时包含明确的新产品、新模型、新技术发布，可以保留，但理由必须写产品或技术点，不要写融资本身。
- 公众号资讯优先级高于网页资讯；但如果公众号资讯属于招聘、融资、会议报名、低质量营销等低价值内容，仍应拒绝。
...
- “客户端 + AI”不是单独分类：如果是操作系统、超级 App、第三方接入、系统级 AI 能力，归入“AI资讯”；如果主要载体是硬件终端或设备能力，归入“智能硬件”。
...
- 若 `AI资讯` 超过 10 条，按 `importance` 和输入顺序截断到 10 条。
- 若 `智能硬件` 超过 4 条，按 `importance` 和输入顺序截断到 4 条。
...
- 客户端 + AI 方向的新动态、新产品、新功能、新接入方式，优先级最高
...
11. 未来发生的时间必须保留未来语气，例如“将于、计划、预计、即将”；不得把“将于6月1日起”改写成已经发生。
12. 不得替原文拔高评价或下行业结论，避免使用“史诗级、重大突破、成熟阶段、行业一流、标杆事件、战略卡位、全球竞争、全面落地”等判断性表达；除非输入明确给出，并且必须加来源限定，如“公司称”“原文称”“被视为”。
13. 如果素材信息不足，只写已知事实，并在 risk_flags 中标记 "insufficient_material"。
14. 保留关键专有名词，如 OpenAI、Claude、DeepSeek、通义千问、Agent、GitHub 等。
15. 输出只允许 JSON，不要 markdown，不要解释。
...
一期对来源保留优先级做轻量区分：微信公众号资讯优先级高于网页资讯。该优先级只在规则过滤之后生效，不保护低价值公众号内容；招聘、融资、会议报名、低质量营销等仍按规则过滤和 LLM 筛选正常剔除。对于重复或语义相似内容，程序层面优先保留公众号版本；LLM 全局筛选阶段也会看到 `source_type=公众号/网页`，用于在质量相近时优先选择公众号来源。
